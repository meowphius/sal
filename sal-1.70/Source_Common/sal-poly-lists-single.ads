-- Abstract :
--
-- A generic singly linked list, with useful operations, allowing definite
-- or indefinite, limited or non-limited item types. All reasonable
-- operations on lists are defined; see child package Iterators for
-- iterator operations.
--
-- List_Type is Controlled to recover storage when a List goes out of scope.
-- List_Type is Limited because I don't want to test a deep copy (but I
-- will someday :).
--
...
generic
   ...
   -- If Item_Type is definite non-limited, Item_Node_Type should just
   -- be Item_Type. Then To_Item_Node should just return Item, and
   -- Free_Item should be null (and both should be inlined). See
