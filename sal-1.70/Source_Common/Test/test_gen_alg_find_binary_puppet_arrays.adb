...
is
   ...
   List : aliased Puppet_Array_Type :=
      (1 => Allocate (Muppet_2_5),
       2 => Allocate (Beanie_0),
       3 => Allocate (Beanie_4));
   -- Need explicit initialization, rather than constrained bounds, to
   -- allow 'access to match an access to unchecked array type.
...
begin
   ...
   -- this just shows we can use plain array containers if we want to.
