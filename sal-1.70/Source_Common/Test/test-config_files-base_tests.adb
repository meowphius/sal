...
package body Test.Config_Files.Base_Tests is
   ...
   is
      ...
      Result_Boolean : constant Boolean := Read (Config, Key, False, Error_Handling => Raise_Exception);
   begin
   ...
   is
      ...
      Result : constant Duration := Read (Config, Key, 0.0, Error_Handling => Raise_Exception);
   begin
      AUnit.Assertions.Assert
        (Result = Expected,
         Key & ": " &
   ...
   begin
      Register_Routine (T, Test_Boolean_Value'Access,
                        "Write, read Boolean value");

      Register_Routine (T, Test_Duration_Value'Access,
                        "Write, read Duration value");

      Register_Routine (T, Test_File_Name'Access,
                        "Write, read File_Name");

      Register_Routine (T, Test_Float_Value'Access,
                        "Write, read Float value");

      Register_Routine (T, Test_Int_Value'Access,
                        "Write, read int value");

      Register_Routine (T, Test_Quoted_String_Value'Access,
                        "Write, read quoted String value");

      Register_Routine (T, Test_String_Value'Access,
                        "Write, read String value");

      Register_Routine (T, Test_Unsigned_Value'Access,
                        "Write, read unsigned value");

      --  Test_Flush_Reopen must be last, because it relies on values
      --  written by the other routines.
      Register_Routine (T, Test_Flush_Reopen'Access,
                        "Flush and reopen file");
   ...
   begin
      ...
      AUnit.Assertions.Assert
        (Found.all = SAL.Config_Files.Writeable_File_Name (Config),
         "Expecting '" & Test.Config_Files.Base_Tests.File_Name & "' " &
