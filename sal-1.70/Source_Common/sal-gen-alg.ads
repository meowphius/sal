-- Abstract :
--
-- Root for generic algorithms for a non-abstract container type. See
-- SAL.Poly.Alg for an abstract container type.
--
...
-- Design :
--
-- This package takes the generic formal parameters that are common to most
-- algorithms for one container type, and are provided by all container and
-- iterator types.
--
...
-- or iterator types.
--
-- Copyright (C) 1999, 2000, 2003, 2004 Stephen Leake.  All Rights Reserved.
--
...
generic
   ...
   with function Current (Iterator : in Iterator_Type) return Item_Node_Type is <>;
   with function First (Container : in Container_Type) return Iterator_Type is <>;
   with function Last (Container : in Container_Type) return Iterator_Type is <>;
   with function None (Container : in Container_Type) return Iterator_Type is <>;
   with function Is_Null (Iterator : in Iterator_Type) return Boolean is <>;
   with procedure Next_Procedure (Iterator : in out Iterator_Type);
   with function Next_Function (Iterator : in Iterator_Type) return Iterator_Type;
