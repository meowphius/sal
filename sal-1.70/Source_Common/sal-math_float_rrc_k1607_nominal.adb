...
package body SAL.Math_Float_RRC_K1607_Nominal is
   ...
   is
      use SAL.Math_Float.Scalar, SAL.Math_Float.DOF_3, SAL.Math_Float.DOF_6;

      -- Mass values are from [2]. The pitch joint modules are broken at the
      -- joint and each piece measured independently. The roll joint modules are
      -- not; the mass of the distal (moving) portion is included in the CG and X
      -- and Y moments of inertia of the proximal link.  The Z moment of inertia
      -- is given, and included in the distal link. This gives the correct total
      -- joint gravity torque and inertia, although individual bearing wrenches
      -- will be off.
      --
      -- First, we record the numbers directly from the document, adding the
      -- distal Izz from the roll modules to the appropriate pitch module clevis.

      -- the base plate and housing don't move, so we don't represent them.

      -- RRC mass values are in Pounds (sigh), and inertia numbers are in
      -- units of Pound-Inch-Second^2 (sigh^2).
