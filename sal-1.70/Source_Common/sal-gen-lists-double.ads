...
--
-- Since List_Type is not Controlled, the user must recover list storage
-- when a list goes out of scope.
--
-- See SAL.Poly.Lists.Double for a Controlled list.
--
-- Item access functions return Item_Node_Type, which may be Item_Type or
-- an access type, depending on the instantiation choices. We don't return
-- Item_Access_Type, because it is impossible for the user to provide
-- 'function To_Access (Item : in out Item_Node_Type) return
-- Item_Access_Type', since it might use copy-in/copy-out parameter
-- passing, which would then return the address of Item on the stack, which
-- is destroyed on return. Thus if the user needs to modify elements in
-- place, they must choose Item_Node_Type to be an access type.
--
-- Because the child package Iterators supports Splice, we don't store a
-- count in list; that would force Splice to count the items in the splice,
-- which would make it not constant time.
--
