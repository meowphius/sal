...
package SAL.Gen.Lists.Double.Iterators is
   ...
   -- Advance to next/previous Item in list. If at last/first item, point
   -- to no item.
   --
   ...
   -- Insert a copy of all items in range [First .. Last] in Source before
   -- Before in Dest.
   --
   ...
   -- case, items from Before thru Last on Last's list have been copied.
   --
   -- Raises Iterator_Error if First not before Last on same list. In this
   -- case, items from Last thru Head on Last's list have been copied.
   --
   ...
   -- Delete Iterator's current item from List. Iterator then points to
   ...
   --
   -- If First, Last both null, delete entire list. If list is empty, no
   -- effect.
   --
   -- Raises Constraint_Error if First not before Last on same list. In
   -- this case, items from First thru Tail on First's list have been
   -- deleted.
   --
   ...
   procedure Insert_After
      (List   : in out List_Type;
       ...
       Item   : in     Item_Type;
       Copies : in     Natural         := 1);
   -- Insert To_Item_Node (Item) after After's current position in List,
   -- Copies times. Later copies are inserted after earlier copies.
   --
   ...
   procedure Insert_Before
      (List   : in out List_Type;
       ...
       Item   : in     Item_Type;
       Copies : in     Natural         := 1);
   -- Insert To_Item_Node (Item) before Iterator's current position in
   -- List, Copies times. Later copies are inserted before earlier copies.
   --
   ...
   --
   -- Does not detect if [First, Last] contains Before, or if First not
   -- before Last on same list.
   ...
   -- position in Dest, removing from their current list. Before may be
   -- from the same list as First and Last, as long as it is not between
   -- them.
   --
