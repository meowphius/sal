...
package SAL.Poly.Binary_Trees.Sorted.Iterators is
   ...
   procedure Finalize (Iterator : in out Iterator_Type);
   -- Deallocate the node stack.

   procedure Adjust (Iterator : in out Iterator_Type);
   -- Copy the node stack.
   ...
   -- Do binary tree search, return Iterator pointing to item with key Key. If
   -- there is no such item, Iterator points to no item.

   function First (Tree : in Tree_Type'class) return Iterator_Type;
   -- Return iterator that points to First element in Tree, or none if Tree
   -- is empty.
   ...
   -- Set Iterator to point to next item with same key as current item. If
   -- there is no such item, Iterator points to no item, and Is_Done is
   -- True.
   ...
   -- Advance to next item in Tree. If at last item, point to no item, mark
   -- Done True.
