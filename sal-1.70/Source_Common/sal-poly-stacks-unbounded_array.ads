...
generic
   ...
   -- Stack starts with space for Initial_Size items. When it is filled,
   -- space is doubled. When it is emptied to one-quarter, space is
   -- halved, but never below Initial_Size.
   ...
   -- for a global array type should be <some_global_access_type>'Storage_Pool.
   -- Default for a local array type, which should be reclaimed when it
   -- goes out of scope, is implementation defined (sigh).
